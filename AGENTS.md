...
# === Cookies ===
COOKIE_SECURE=false           # true, если за HTTPS, false для HTTP
ENABLE_BROWSER_COOKIE_SOURCE=false  # Включить browser sidecar
DEFAULT_COOKIE_SOURCE=uploaded_file # uploaded_file | browser_session
BROWSER_COOKIE_SERVICE_URL=         # URL browser sidecar (http://youbox-browser:3808)
BROWSER_COOKIE_EXPORT_PATH=         # Путь для экспортированных cookies
...
│   ├── downloader.ts  # yt-dlp: extract + download + playlist + cookies (через cookie-source)
│   ├── cookie-source.ts # Cookie Source Manager: uploaded_file / browser_session
...
│   ├── health.ts      # Health check (yt-dlp, ffmpeg, БД, cookie source)
...
│   ├── env.ts         # Валидация ENV + cookie source vars
│   └── theme-context.tsx # React Context для dark/light темы
├── proxy.ts           # Auth proxy (Next.js 16) — защита API роутов
├── instrumentation.ts # Запуск воркера + cookie source init при старте
├── Dockerfile         # Multi-stage, HOME=/tmp, USER youbox
├── docker-compose.yml # Bind mount ./data:/data, env_file: .env, browser sidecar (profile)
├── browser-sidecar/   # Опциональный browser sidecar (Chromium + Playwright)
│   ├── Dockerfile
│   ├── package.json
│   └── src/server.js  # HTTP API: /status, /export, /validate, /health
...
|------|--------|------|---------|
| `/api/cookie-source` | GET | Да | Сводка по источникам cookies |
| `/api/cookie-source` | DELETE | Да | Удалить источник |
| `/api/cookie-source/upload` | POST | Да | Загрузить cookies.txt |
| `/api/cookie-source/:id/activate` | POST | Да | Активировать источник |
| `/api/cookie-source/:id/validate` | POST | Да | Валидация + глубокая проверка |
...
- **Два типа источников**: `uploaded_file` (загрузка через UI) и `browser_session` (sidecar с Chromium)
- **Active source**: только один, переключение через UI или `activateSource(id)`
- **Fallback**: если активный источник недоступен → `YT_COOKIES_FILE` из env → без cookies
- **Валидация**: базовая (проверка формата Netscape) + глубокая (тестовый запрос к YouTube через yt-dlp)
- **Browser sidecar**: опциональный Docker контейнер, profile `browser`, Playwright + Chromium
- **ENV toggle**: `ENABLE_BROWSER_COOKIE_SOURCE=true/false`, `DEFAULT_COOKIE_SOURCE=uploaded_file|browser_session`
- **Безопасность**: содержимое cookies не логируется, файлы с правами 600, пути не раскрываются в API
...
- **Миграции**: user_version pragma, шесть версий (v1: schema, v2: login_attempts+mode, v3: format_id, v4: progress_*, v5: cookie_sources, v6: app_settings)
