...
3. **Сессии**: UUID v4, хранится в таблице `sessions` с expiry. Кука: HTTP-only, SameSite=Lax, Secure=env.COOKIE_SECURE (по умолчанию true), Path=/
...
# Cookies
COOKIE_SECURE=false           # true, если за HTTPS (иначе кука не отправляется по HTTP)
...
- `HOME=/tmp` — yt-dlp пишет кеш в tmpfs
- Запуск: `node server.js` от пользователя `youbox`
...
services:
  youbox:
    ...
    volumes:
      - ./data:/data                           # bind mount для БД и загрузок
      - ${YT_COOKIES_FILE:-./data/cookies.txt}:/cookies/cookies.txt
    security_opt:
